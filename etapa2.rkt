...
(define (get-suffixes text)
  (cond
   ((= (length text) 0) '())
    ; Adaugam la fiecare pas cuvantul curent si vom parcurge recursiv pana cand nu mai raman litere
...
(define (get-ch-words words ch)
  (filter (lambda (x)
            (and (not (null? x)) ; Conditia sa nu avem cuvinte vide
                 (char=? (car x) ch))) ; Extragem din lista doar cuvintele care incep cu caracterul respectiv
          words))
...
(define (ast-func suffixes)
  ; Formam lista corespunzatoare
  ; Initial adaugam eticheta, care va fi prima litera din primul cuvant din lista
  ; Pentru a adauga celelalte cuvinte din lista, vom folosi foldr
  ; Funtia binara lambda unde x este elementul curent
...
(define (cst-func suffixes)
  ; Adaugam prefixul comun, cu ajutorul functiei longest-common-prefix
  ; Pentru a crea restul cuvintelor, folosim foldr
  ; Pentru fiecare x, aplicam functia longest-common-prefix care va primi prefixul comun si cuvantul curent: x
  ; Adaugam ceea ce ramane din cuvantul curent x, dupa eliminarea prefixului comun
  (define prefix (longest-common-prefix-of-list suffixes))
  (cons prefix
        (foldr (lambda (x acc)
                 (cons (car (cdr (cdr (longest-common-prefix prefix x)))) acc))
               '()
               suffixes)))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (define arbore
     (map (lambda (ch) ; Pentru fiecare litera din alfabet vom crea o lista cu sufixele care incep cu litera corespunzatoare
                 (define list-suffixes (get-ch-words suffixes ch)) ; Lista cu sufixele care incep cu litera corespunzatoare
                 (cond
                   ((null? list-suffixes) '())
                   ; Pentru lista de sufixe vom apela functia labeling-func
                   ...
                   (else (cons (car (labeling-func list-suffixes))
                               (suffixes->st labeling-func (cdr (labeling-func list-suffixes)) alphabet)))))
                 
                alphabet))
  ; Eliminam listele vide din arbore
 (foldr (lambda (x acc)
           (cond
             ((null? x) acc)
             (else (cons x acc))))
...
(define (alphabet-func text)
  (sort (remove-duplicates (cons #\$ text)) char<?))

; Functie care determina sufixele textului
; Adaugam pentru fiecare sufix $ la final
; La final adaugam si sufixul vid, reprezentat de $
...
(define (text->st labeling-func) ; Va primi ca parametru functia de etichetare
  ; Returneaza o functie care asteapta urmatorul argument: text
    (lambda (text)
      (define alphabet (alphabet-func text))
      (define suffixes (suffixes-func text))
       (suffixes->st labeling-func suffixes alphabet))) ; Obtinem arborele corespunzator
...
; asociat textului.
(define text->ast
  (text->st ast-func)) ; Apelam text->ast cu functia de etichetare corespunzatoare
